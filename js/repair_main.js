...
// Fix the corrupted setupUI click listener
const corruptedRegex = /this\.loadVirus\(id, this\.currentMode\);[\s\S]*?this\.switchMode\(mode\);/;
const fixedSetupUI = \`this.loadVirus(id, this.currentMode);
        this.resetCamera();
      });
    });
...
code = code.replace(corruptedRegex, fixedSetupUI);

// Fix init() method
const corruptedInitRegex = /init\(\) \{[\s\S]*?setupScene\(\) \{/;
const fixedInit = \`init() {
  ...
  setupScene() {\`;
code = code.replace(corruptedInitRegex, fixedInit);

// Now, correctly inject ARButton logic into setupScene() right before return or end
const setupSceneEndRegex = /this\.renderer\.toneMappingExposure = 1\.1;/;
const arSetupCode = \`this.renderer.toneMappingExposure = 1.1;
...
code = code.replace(setupSceneEndRegex, arSetupCode);
