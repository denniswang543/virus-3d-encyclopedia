window.VirusAnimations = {
  ...
  playT4(scene, t4Group) {
    ...
    if (!this.membraneMesh) {
      ...
      this.membraneMesh.position.y = -3.5;
    }
    scene.add(this.membraneMesh);

    // 取得部位
    const headGroup = t4Group.getObjectByName("t4_headGroup");
    const sheathGroup = t4Group.getObjectByName("t4_sheathGroup");
    const dna = t4Group.getObjectByName("t4_dna");
    const injectDna = t4Group.getObjectByName("t4_injectDna");

    if (!headGroup || !sheathGroup) return;

    // 儲存原始狀態 (如果沒有存過)
    if (headGroup.userData.origY === undefined) {
      headGroup.userData.origY = headGroup.position.y;
      sheathGroup.userData.origY = sheathGroup.position.y;
      sheathGroup.userData.origScaleY = sheathGroup.scale.y;
    }

    // 重設到初始高空狀態
    t4Group.position.set(0, 5, 0);
    headGroup.position.y = headGroup.userData.origY;
    sheathGroup.position.y = sheathGroup.userData.origY;
    sheathGroup.scale.y = sheathGroup.userData.origScaleY;
    
    if (dna) dna.material.opacity = 1;
    if (injectDna) injectDna.material.opacity = 0;

    

    this.startTime = Date.now();
    this.activeAnimation = requestAnimationFrame(() => this.animateT4(t4Group, headGroup, sheathGroup, dna, injectDna));
  },

  animateT4(t4Group, headGroup, sheathGroup, dna, injectDna) {
    ...
    // 階段 1: 降落 (0 ~ 2 秒)
    if (elapsed < 2.0) {
      this.showText("階段一：噬菌體降落於細菌表面尋找受體");
      const progress = elapsed / 2.0;
      // ease-out
      const ease = 1 - Math.pow(1 - progress, 3);
      t4Group.position.y = 5 - (5 * ease);
    } 
    // 階段 2: 尾鞘收縮、中心管穿透 (2 ~ 4 秒)
    else if (elapsed >= 2.0 && elapsed < 4.0) {
      this.showText("階段二：尾鞘強烈收縮，中心針管刺破細胞膜");
      const progress = (elapsed - 2.0) / 2.0;
      const ease = progress < 0.5 ? 2 * progress * progress : 1 - Math.pow(-2 * progress + 2, 2) / 2;
      
      t4Group.position.y = 0;
      // Head group moves down by 1.0 unit
      headGroup.position.y = headGroup.userData.origY - (1.0 * ease);
      
      // Sheath compresses to half size and moves down to stay anchored to baseplate
      sheathGroup.scale.y = 1.0 - (0.5 * ease);
      sheathGroup.position.y = sheathGroup.userData.origY - (0.5 * ease);
    }
    // 階段 3: 注入 DNA (4 ~ 7 秒)
    else if (elapsed >= 4.0 && elapsed < 7.0) {
      this.showText("階段三：頭部加壓，將病毒 DNA 注入宿主細胞內");
      const progress = (elapsed - 4.0) / 3.0;
      
      // Fade out head DNA
      if (dna) {
        dna.material.transparent = true;
        dna.material.opacity = Math.max(0, 1 - progress * 1.5);
      }

      // Show and animate inject line
      if (injectDna) {
        injectDna.material.opacity = Math.min(1, Math.sin(progress * Math.PI) * 2);
        // Pulse color
        injectDna.material.color.setHSL(0.5, 1, 0.5 + 0.5 * Math.sin(elapsed * 10));
      }
    }
    // 階段 4: 完成
    else if (elapsed >= 7.0 && elapsed < 10.0) {
      this.showText("感染完成！DNA 已進入宿主準備複製。");
      if (injectDna) injectDna.material.opacity = 0;
    }
    else {
      // Loop
  ...
  stop(scene) {
    ...
    const el = document.getElementById("anim-subtitle");
    if (el) el.innerText = "";
    
    if (this.membraneMesh && scene) {
      scene.remove(this.membraneMesh);
    }
  }
};
